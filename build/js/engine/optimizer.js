...
function incrementDepth(map, depth) {
    map.set(depth, (map.get(depth) ?? 0) + 1);
}
function depthRecord(map) {
    const result = {};
 ...
 */
export function recommendNextAction(state, data, uniformStatFallback = true, searchOptions = {}) {
    ...
    const targetedActionRequestsByDepth = new Map();
    const targetedActionCacheHitsByDepth = new Map();
    const targetedActionCacheMissesByDepth = new Map();
    ...
    const targetedContinuation = (engine, operation, role, tokensRemaining, phase) => {
        ...
        if (prior) {
            ...
            incrementDepth(targetedActionCacheHitsByDepth, tokensRemaining);
            return prior;
        }
        targetedActionCacheMisses++;
        incrementDepth(targetedActionCacheMissesByDepth, tokensRemaining);
        ...
        for (const operation of state.menu) {
            for (const role of ROLES) {
                ...
                if (state.tokensRemaining > horizon)
                    ...
                    row.note = `${horizon}-token continuation uses deterministic probability stratification (${continuationEntryStrata} entry / ${continuationStrata} fresh-menu strata max).`;
                rows.push(row);
            }
        }
