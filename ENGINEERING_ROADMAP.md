...
Five-slot quality redistribution is now defined and tested: one slot is selected uniformly to decrease, then two distinct recipients are selected uniformly from all unordered pairs among the remaining four slots. The other two slots are unchanged. Existing tier-direction distributions, Tier-I/Tier-V floor/cap waste, and final-state aggregation are preserved. For `legacy_3`, this same generalized definition reduces exactly to one decreased slot and the other two increased.

The final authoritative M6A Node 22 matrix completed 12/12 cases with no benchmark errors or timeouts, followed by a successful complete test run against regenerated artifacts. Relative to `legacy_3`, fresh `expanded_5` optimizer runtime at `t=2` was 1.95× stat-heavy, 4.21× quality-heavy, 2.33× trait-heavy, 3.34× global-quality, and 1.93× target-probability. The isolated stat-heavy `t=1` wall-time ratio was 0.24× and is treated as noisy rather than evidence of a structural speedup. Expanded target-probability `t=2` took about 7.48 s versus about 3.87 s for legacy in the final run.

The dominant effect remains frontier/branch growth rather than terminal-scoring cost: representative stat-heavy one-step reachable outcomes remain 5 → 21 while terminal scoring stays roughly flat. Enabling expanded redistribution also makes the global-quality comparison semantically complete and increases its measured frontier cost.

M6A therefore closes as **Outcome A**: the same versioned engine supports both layouts, legacy compatibility remains intact, and all known expanded operation semantics are represented without a parallel implementation.
