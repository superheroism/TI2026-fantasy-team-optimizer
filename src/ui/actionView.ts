import type { ActionEvaluation, BoardState, MenuState, OptimizerState, RecommendationResult, Role } from '../domain/types.js';
...
export const ACTION_DROPDOWN_GROUP_LABELS = ['Red','Blue','Green','Boosts'] as const;
const COLOR_ACTION_GROUPS = [
  {label:'Red',color:'red'},
  {label:'Blue',color:'blue'},
  {label:'Green',color:'green'},
] as const;
const ACTION_FAMILY_ORDER = ['stat_reroll','quality_reroll','trait_reroll'] as const;

type CatalogAction = (typeof ACTION_CATALOG)[number];

function actionOption(action:CatalogAction,selectedId:string,selectedElsewhere:ReadonlySet<string>):string {
...
export function groupedActionOptions(selectedId:string,selectedElsewhere:ReadonlySet<string>):string {
...
export function renderOperationEditors(menu: MenuState): string {
...
export function utilityDeltaText(delta: number, targetMode: boolean): string {
  return targetMode ? `${delta >= 0 ? '+' : ''}${(delta * 100).toFixed(1)} pp` : `${delta >= 0 ? '+' : ''}${fmt(delta)}`;
}

export function utilityText(value: number, targetMode: boolean): string {
...
function boardRowsForOperation(result: RecommendationResult, operationId: string): ActionEvaluation[] {
  return result.ranking.filter((row): row is ActionEvaluation => row.action.kind === 'board_action' && row.action.operationId === operationId && row.status === 'evaluated');
}
...
export interface ActionResultViewOptions {
  result: RecommendationResult;
  state: OptimizerState;
  board: BoardState;
  menu: MenuState;
  targetSelection: Map<number, Role>;
  onTargetSelectionChanged: () => void;
}

export function renderActionResults(options: ActionResultViewOptions): void {
  ...
  for (const row of allBoardRows) {
    for (const value of [row.outcomeP10Utility, row.outcomeMedianUtility, row.outcomeP90Utility, row.expectedFinalUtility]) {
      if (value !== undefined) rangeValues.push(value - stopUtility);
    }
  }
  ...
  menu.forEach((operation, index) => {
    const card = document.querySelector<HTMLElement>(`.op-card[data-op="${index}"]`);
    const resultElement = document.querySelector<HTMLElement>(`[data-opresult="${index}"]`);
    ...
    if (!selectedRole || !legalRoles.includes(selectedRole)) selectedRole = best.action.kind === 'board_action' ? best.action.banner : legalRoles[0]!;
  ...
  document.querySelectorAll<HTMLButtonElement>('[data-action-target]').forEach(button => button.addEventListener('click', () => {
    ...
    targetSelection.set(Number(indexText), role as Role);
...
export function clearActionResults(targetSelection: Map<number, Role>, message = 'Optimize the board to compare targets and reroll outcomes.'): void {
  ...
  const next = document.querySelector<HTMLButtonElement>('#next-roll');
