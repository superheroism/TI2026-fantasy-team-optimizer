import type { DataBundle, PlayerProfile, QuantilePoint, Role, StatName } from '../domain/types.js';
...
function curve(base:number, volatility:number): QuantilePoint[] { return Q.map((q,i)=>({q,value:Math.max(0,base*(1+volatility*([-1.3,-0.55,0,0.55,1.3][i]!)))})); }
const roleBase: Record<Role,Partial<Record<StatName,number>>> = {
...
function profile(id:string,name:string,team:string,role:Role,factor:number,vol:number):PlayerProfile{
  ...
  for(const stat of allowed){const base=roleBase[role][stat]??200;statQuantiles[stat]=curve(base*factor,vol);}
...
const statLists:Record<Role,StatName[]>={
...
function corr(stats:StatName[],rho:number){return {stats,spearman:stats.map((_,i)=>stats.map((__,j)=>i===j?1:rho))};}
export const demoData:DataBundle={
