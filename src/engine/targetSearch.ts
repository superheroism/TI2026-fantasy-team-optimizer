...
export function choosePreparedTargetSearch<T>(
  ...
  const diagnosticStart = diagnosticsEnabled ? nowMs() : 0;
  if (diagnosticsEnabled) diagnostics.searchesConsidered++;
  ...
  const searchUpperExpected =
    firstSummary.maxExpected + secondSummary.maxExpected + thirdSummary.maxExpected;

  if (!optimisticBoundCanBeat(
    firstSummary.maxSamples,
    secondSummary.maxSamples,
    thirdSummary.maxSamples,
    targetScore,
    searchUpperExpected,
    incumbentHits,
    incumbentExpected,
...

  {
    ...
    for (let i = 0; i < n; i++) {
      if (a.samples[i]! + b.samples[i]! + c.samples[i]! >= targetScore) hits++;
    }
  ...
  for (const a of first) {
    ...
    const firstUpperExpected =
      a.expected + secondSummary.maxExpected + thirdSummary.maxExpected;
