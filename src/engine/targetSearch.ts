...
export interface TargetSearchDiagnostics {
  ...
  suffixScenarioChecks: number;
  survivingPairSampleBuilds: number;
  thirdCandidatesVisited: number;
  maxThirdCandidatesVisited: number;
  suffixBoundCalls: number;
  suffixBoundPruned: number;
  suffixThirdCandidatesSkipped: number;
  pairGroupCacheHits: number;
  pairGroupCacheMisses: number;
  pairSampleCacheHits: number;
  pairSampleCacheMisses: number;
  pairSampleCacheBuilds: number;
  pairCacheResets: number;
  pairCacheEstimatedBytes: number;
  suffixCacheHits: number;
  suffixCacheMisses: number;
  suffixCacheBuilds: number;
  suffixCacheResets: number;
  suffixCacheEstimatedBytes: number;
  candidatePreparationMs: number;
  pairSampleBuildMs: number;
  suffixSummaryBuildMs: number;
...
interface SuffixBlockSummary {
  bounds: Array<Float64Array | undefined>;
  maxExpected: Float64Array;
  estimatedBytes: number;
}

interface PairGroupCache {
  uses: number;
  samples: Array<Float64Array | undefined> | undefined;
  secondLength: number;
  disabled: boolean;
}
...
let seenPreparedGroups: [WeakSet<object>, WeakSet<object>, WeakSet<object>] = [
  new WeakSet(),
  new WeakSet(),
  new WeakSet(),
];
let seenPreparedTuples = new WeakMap<object, WeakMap<object, WeakSet<object>>>();
let seenPreparedPairs: [WeakMap<object, WeakSet<object>>, WeakMap<object, WeakSet<object>>, WeakMap<object, WeakSet<object>>] = [
  new WeakMap(),
  new WeakMap(),
  new WeakMap(),
];
...
let preparedSummaryCache = new WeakMap<object, PreparedCandidateSummary>();
let suffixBlockCache = new WeakMap<object, SuffixBlockSummary>();
let suffixCacheEstimatedBytes = 0;
let pairGroupCache = new WeakMap<object, WeakMap<object, PairGroupCache>>();
let pairCacheEstimatedBytes = 0;
const pairScratchPool = new Map<number, Float64Array[]>();

export function clearTargetSearchOptimizationCaches(): void {
...
function dominates<T>(
  a: PreparedTargetSearchCandidate<T>,
  b: PreparedTargetSearchCandidate<T>,
): boolean {
...
function pruneDominated<T>(
  candidates: PreparedTargetSearchCandidate<T>[],
): PreparedTargetSearchCandidate<T>[] {
...
export function prepareTargetCandidates<T>(
  ...
  const prepared = pruneDominated(
    candidates.map((candidate) => prepareCandidate(candidate, iterations)),
  );
...
function summarizePreparedCandidates<T>(
  candidates: readonly PreparedTargetSearchCandidate<T>[],
  iterations: number,
...
function suffixBoundStart(index: number): boolean {
  return index === 1 || (index >= SUFFIX_BLOCK_SIZE && index % SUFFIX_BLOCK_SIZE === 0);
}

function summarizeSuffixBlocks<T>(
  candidates: readonly PreparedTargetSearchCandidate<T>[],
  iterations: number,
): SuffixBlockSummary | undefined {
  const prior = suffixBlockCache.get(candidates);
  if (prior) {
    if (diagnosticsEnabled) diagnostics.suffixCacheHits++;
    return prior;
  }
  if (diagnosticsEnabled) diagnostics.suffixCacheMisses++;

  const boundCount = candidates.length <= 1
    ? 0
    : 1 + Math.max(0, Math.floor((candidates.length - 1) / SUFFIX_BLOCK_SIZE));
  const estimatedBytes =
    boundCount * iterations * Float64Array.BYTES_PER_ELEMENT
    + candidates.length * Float64Array.BYTES_PER_ELEMENT
    + candidates.length * 8;
  if (estimatedBytes === 0) return undefined;

  if (suffixCacheEstimatedBytes + estimatedBytes > SUFFIX_CACHE_BYTE_LIMIT) {
    suffixBlockCache = new WeakMap();
    suffixCacheEstimatedBytes = 0;
    if (diagnosticsEnabled) diagnostics.suffixCacheResets++;
  }

  const started = diagnosticsEnabled ? nowMs() : 0;
  const bounds = new Array<Float64Array | undefined>(candidates.length);
  ...
  for (let candidateIndex = candidates.length - 1; candidateIndex >= 0; candidateIndex--) {
    ...
    if (candidate.expected > runningExpected) runningExpected = candidate.expected;
    for (let i = 0; i < iterations; i++) {
      if (candidate.samples[i]! > running[i]!) running[i] = candidate.samples[i]!;
    }
...
function optimisticBoundCanBeat(
  ...
  bestHits: number,
  bestExpected: number,
): { canBeat: boolean; checked: number } {
  let hits = 0;
  const n = a.length;
  ...
  for (let i = 0; i < n; i++) {
    ...
    if (maximumPossibleHits < bestHits) return { canBeat: false, checked };
...
function trackIdentity(groups: PreparedTargetSearchGroups<unknown>): void {
  ...
  const pairValues: readonly [object, object][] = [[first, second], [second, third], [first, third]];
...
function pairCacheFor<T>(
  first: readonly PreparedTargetSearchCandidate<T>[],
  second: readonly PreparedTargetSearchCandidate<T>[],
): PairGroupCache {
  ...
  if (!cache) {
    ...
    if (diagnosticsEnabled) diagnostics.pairGroupCacheMisses++;
    return cache;
  }
  cache.uses++;
  if (diagnosticsEnabled) diagnostics.pairGroupCacheHits++;
  if (cache.uses === 2 && cache.samples === undefined && !cache.disabled) {
    ...
    if (pairCacheEstimatedBytes + pointerBytes <= PAIR_CACHE_BYTE_LIMIT) {
      cache.samples = new Array<Float64Array | undefined>(first.length * second.length);
      pairCacheEstimatedBytes += pointerBytes;
      if (diagnosticsEnabled) diagnostics.pairCacheEstimatedBytes = pairCacheEstimatedBytes;
    } else {
      pairGroupCache = new WeakMap();
      pairCacheEstimatedBytes = 0;
      cache.disabled = true;
...
function storePairSamples(
  cache: PairGroupCache,
  index: number,
  samples: Float64Array,
): Float64Array {
  if (!cache.samples) return samples;
...
function materializeChoice<T>(
  bestHits: number,
  bestExpected: number,
  best: [PreparedTargetSearchCandidate<T>, PreparedTargetSearchCandidate<T>, PreparedTargetSearchCandidate<T>] | undefined,
  n: number,
...
export function choosePreparedTargetSearchReference<T>(
  ...
  if (!first.length || !second.length || !third.length) return undefined;
  ...
  if (!optimisticBoundCanBeat(firstSummary.maxSamples, secondSummary.maxSamples, thirdSummary.maxSamples, targetScore, searchUpperExpected, incumbentHits, incumbentExpected).canBeat) return undefined;
  ...
  let seedHits = 0;
  for (let i = 0; i < n; i++) if (seedA.samples[i]! + seedB.samples[i]! + seedC.samples[i]! >= targetScore) seedHits++;
  const seedExpected = seedA.expected + seedB.expected + seedC.expected;
  if (betterThan(seedHits, seedExpected, bestHits, bestExpected)) {
    bestHits = seedHits; bestExpected = seedExpected; best = [seedA, seedB, seedC];
  }
  ...
  for (const a of first) {
    ...
    if (!optimisticBoundCanBeat(a.samples, secondSummary.maxSamples, thirdSummary.maxSamples, targetScore, firstUpperExpected, bestHits, bestExpected).canBeat) continue;
    for (const b of second) {
      const pairExpected = a.expected + b.expected;
      const pairUpperExpected = pairExpected + thirdSummary.maxExpected;
      if (!optimisticBoundCanBeat(a.samples, b.samples, thirdSummary.maxSamples, targetScore, pairUpperExpected, bestHits, bestExpected).canBeat) continue;
      ...
      for (const c of third) {
        ...
        if (betterThan(hits, expected, bestHits, bestExpected)) {
          bestHits = hits; bestExpected = expected; best = [a, b, c];
        }
      }
    }
  }
...
export function choosePreparedTargetSearch<T>(
  ...
  if (collect) {
    ...
    trackIdentity(groups as PreparedTargetSearchGroups<unknown>);
  }

  if (!first.length || !second.length || !third.length) {
    if (collect) diagnostics.combinatorialSearchMs += nowMs() - diagnosticStart;
    return undefined;
  }
  ...
  try {
    ...
    for (let ai = 0; ai < first.length; ai++) {
      ...
      if (!firstBound.canBeat) { firstPruned++; continue; }
      ...
      for (let bi = 0; bi < second.length; bi++) {
        ...
        if (pairSamples) {
          ...
          for (let i = 0; i < n; i++) {
            checked++;
            if (pairSamples[i]! + thirdMax[i]! >= targetScore) hits++;
            const maximumPossibleHits = hits + n - checked;
            if (maximumPossibleHits < bestHits || (maximumPossibleHits === bestHits && pairUpperExpected <= bestExpected + EPSILON)) {
              pairCanBeat = false; break;
            }
          }
          pairChecks += checked;
          if (pairCanBeat) pairCanBeat = boundCanBeat(hits, pairUpperExpected, bestHits, bestExpected);
        } else {
          pairSampleMisses++;
          let hits = 0, checked = 0;
          ...
          for (let i = 0; i < n; i++) {
            checked++;
            if (as[i]! + bs[i]! + thirdMax[i]! >= targetScore) hits++;
            const maximumPossibleHits = hits + n - checked;
            if (maximumPossibleHits < bestHits || (maximumPossibleHits === bestHits && pairUpperExpected <= bestExpected + EPSILON)) {
              pairCanBeat = false; break;
            }
        ...
        if (!pairCanBeat) { pairPruned++; continue; }

        if (!pairSamples) {
          const buildStart = collect ? nowMs() : 0;
          const as = a.samples, bs = b.samples;
          for (let i = 0; i < n; i++) scratch[i] = as[i]! + bs[i]!;
          pairBuilds++;
          if (collect) pairBuildMs += nowMs() - buildStart;
          if (pairCache.samples) pairSamples = storePairSamples(pairCache, cacheIndex, scratch);
          else pairSamples = scratch;
        }

        let visitedThisPair = 0;
        for (let ci = 0; ci < third.length; ci++) {
          const c = third[ci]!;
          triplesConsidered++; visitedThisPair++;
          const expected = pairExpected + c.expected;
          let hits = 0, checked = 0, abandoned = false;
          const cs = c.samples;
          for (let i = 0; i < n; i++) {
            checked++;
            if (pairSamples[i]! + cs[i]! >= targetScore) hits++;
            const maximumPossibleHits = hits + n - checked;
            if (maximumPossibleHits < bestHits || (maximumPossibleHits === bestHits && expected <= bestExpected + EPSILON)) {
              abandoned = true; break;
            }
          ...
          if (canBeat) canBeat = boundCanBeat(upperHits, suffixUpperExpected, bestHits, bestExpected);
          if (!canBeat) {
            suffixPruned++;
        ...
        if (visitedThisPair > maxThirdVisited) maxThirdVisited = visitedThisPair;
      }
    }
  } finally {
  ...
  if (collect) {
    ...
    if (maxThirdVisited > diagnostics.maxThirdCandidatesVisited) diagnostics.maxThirdCandidatesVisited = maxThirdVisited;
