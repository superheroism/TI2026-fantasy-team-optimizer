...
export function createEngineExpectedScorer(
  ...
  const frontierMemo:Record<Role,Map<BannerStateID,RoleFrontierVector>>={core:new Map(),mid:new Map(),support:new Map()};
  ...
  const vectorizeFrontier=(frontier:RoleFrontier):RoleFrontierVector=>{
    ...
    for(const entry of frontier){const index=prefixIndex.get(entry.prefixId);if(index!==undefined)values[index]=entry.adjustedExpected;}
    return values;
  };

  const frontierFor=(role:Role,id:BannerStateID):RoleFrontierVector=>{
    const prior=frontierMemo[role].get(id);if(prior){bannerCacheHits++;return prior;}
    bannerCacheMisses++;
    const frontier=vectorizeFrontier(rolePrefixFrontier(role,bannerFor(role,id),data,iterations,layoutId));
    frontierMemo[role].set(id,frontier);return frontier;
  };
