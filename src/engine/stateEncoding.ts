...
function assertSlot(layoutId:BoardLayoutId,role:Role,position:number,emblem:EmblemState):void {
  ...
  if(emblem.color!==slot.color){const prefix=layoutId==='legacy_3'?`${role} slot ${position}`:`${layoutId}/${role} slot ${position}`;throw new Error(`Expected ${prefix} color ${slot.color}, got ${emblem.color}.`);}
}
