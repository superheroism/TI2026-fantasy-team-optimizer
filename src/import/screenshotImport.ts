...
export interface ScreenshotFieldConfidence { path: string; confidence: number; }
...
export interface RawScreenshotImport {
  ...
  operationIds: [string | null, string | null, string | null];
...
export function screenshotImportRequest(fileDataUrl: string, data: DataBundle): ScreenshotImportRequest {
  const teamsByRole = Object.fromEntries(ROLES.map(role => [role, [...new Set(data.players.filter(player => player.role === role).map(player => player.team))].sort()])) as Record<Role, string[]>;
...
export function validateScreenshotImport(raw: unknown, data: DataBundle, currentBoard: BoardState, currentMenu: MenuState): ValidatedScreenshotImport {
  ...
  for (const role of ROLES) {
    ...
    if (rawEmblems.length !== slots.length) throw new Error(`${role} has ${rawEmblems.length} parsed emblems but ${layoutId} requires ${slots.length}.`);
    const emblems = slots.map((slot, index) => {
      ...
      if (typeof candidate.stat !== 'string' || !isLegalStat(slot.color, candidate.stat as StatName)) throw new Error(`${role} emblem ${index + 1} returned an illegal ${slot.color} stat.`);
      return { id:`${role}-${slot.index}`, position:slot.index, color:slot.color, stat:candidate.stat as StatName, qualityTier:asQualityTier(candidate.qualityTier, `${role} emblem ${index + 1}`), trait:asTrait(candidate.trait, `${role} emblem ${index + 1}`) } satisfies EmblemState;
    }) as BannerEmblems;
  ...
  raw.operationIds.forEach((value, index) => {
    if (value === null) {
      ...
      if (!fieldConfidence.some(field => field.path === `operationIds.${index}`)) fieldConfidence.push({ path:`operationIds.${index}`, confidence:0 });
  ...
  const visibleIds = raw.operationIds.filter((value): value is string => typeof value === 'string');
  if (new Set(visibleIds).size !== visibleIds.length) throw new Error('Screenshot parser returned duplicate offered actions.');
  const menu = resolvedIds.map(id => cloneAction(ACTION_BY_ID.get(id)!)) as MenuState;
  ...
  const result: ValidatedScreenshotImport = { board, menu, warnings, lowConfidenceFields, requiresReview:warnings.length > 0 || lowConfidenceFields.length > 0 };
...
export async function fileToScreenshotDataUrl(file: File, maxDimension = 1800): Promise<string> {
  ...
  const context = canvas.getContext('2d'); if (!context) throw new Error('Canvas image processing is unavailable in this browser.');
