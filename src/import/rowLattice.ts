...
export type LayoutEvidence =
  | { kind:'expanded_5'; confidence:number }
  | { kind:'legacy_3'; confidence:number }
  | { kind:'unresolved'; confidence:number };

export interface RowLatticeFit {
  rows:number[];
  origin:number;
  pitch:number;
  matchedRows:number;
...
function finiteRows(values:readonly number[],height:number):number[]{
  ...
  for(const value of sorted){
    ...
    if(last===undefined||value-last>tolerance)clustered.push(value);
    else clustered[clustered.length-1]=(last+value)/2;
  }
  return clustered;
}

export function classifyLayoutEvidence(
  rowsByRole:Readonly<Record<Role,readonly number[]>>,
  ...
  if(global>=5||counts.some(count=>count>=5))return{kind:'expanded_5',confidence:.99};
  if(global===4||rolesAtLeast4>=2)return{kind:'expanded_5',confidence:.94};
  if(rolesAtLeast4===1&&rolesAtLeast3>=2)return{kind:'expanded_5',confidence:.88};
  if(global===3&&rolesAtLeast3>=2&&rolesAtLeast4===0)return{kind:'legacy_3',confidence:.94};
  if(global===3&&rolesAtLeast3===1&&Math.max(...counts)<=3)return{kind:'legacy_3',confidence:.82};
...
export function fitRowLattice(
  ...
  if(observed.length<minimumEvidence)return null;
  const minPitch=Math.max(18,height*.045),maxPitch=height*.26;
  let best:{origin:number;pitch:number;matchedRows:number;residual:number;score:number}|null=null;

  for(let a=0;a<observed.length-1;a++)for(let b=a+1;b<observed.length;b++){
    const delta=observed[b]!-observed[a]!;
    for(let ia=0;ia<rowCount-1;ia++)for(let ib=ia+1;ib<rowCount;ib++){
      const pitch=delta/(ib-ia);
      if(!Number.isFinite(pitch)||pitch<minPitch||pitch>maxPitch)continue;
      const origin=(observed[a]!-pitch*ia+observed[b]!-pitch*ib)/2;
      const final=origin+pitch*(rowCount-1);
      if(origin<0||final>height)continue;
      const tolerance=Math.max(7,Math.min(26,pitch*.22));
      const used=new Set<number>();
      let residual=0,matched=0,minIndex:number=rowCount,maxIndex=-1;
      for(const value of observed){
        const index=Math.round((value-origin)/pitch);
        if(index<0||index>=rowCount||used.has(index))continue;
        const expected=origin+pitch*index,error=Math.abs(value-expected);
        if(error>tolerance)continue;
        used.add(index);matched++;residual+=error/tolerance;
        minIndex=Math.min(minIndex,index);maxIndex=Math.max(maxIndex,index);
      }
      if(matched<minimumEvidence||maxIndex-minIndex<2)continue;
      const normalizedResidual=residual/matched;
      const score=matched*2-normalizedResidual-(Math.abs((final-origin)/height)*.02);
      if(!best||score>best.score)best={origin,pitch,matchedRows:matched,residual:normalizedResidual,score};
    }
  }
  if(!best)return null;
  const rows=Array.from({length:rowCount},(_,index)=>best!.origin+best!.pitch*index);
  if(rows.some((row,index)=>!Number.isFinite(row)||row<0||row>height||(index>0&&row<=rows[index-1]!)))return null;
 ...
 */
export function fitTierAnchoredLattice(
  observations:readonly number[],
  rowCount:3|5,
  height:number,
  reference?:Pick<RowLatticeFit,'origin'|'pitch'>|null,
):RowLatticeFit|null{
  if(!Number.isFinite(height)||height<=0)return null;
  const observed=finiteRows(observations,height);
  ...
  for(let divisor=1;divisor<rowCount;divisor++){
    ...
    if(!pitches.some(existing=>Math.abs(existing-pitch)<.5))pitches.push(pitch);
  }

  let best:{origin:number;pitch:number;steps:number;residual:number;score:number}|null=null;
  for(const pitch of pitches){
    ...
    for(const gap of gaps){
      ...
      if(error>.22){valid=false;break;}
      stepCounts.push(steps);totalSteps+=steps;residual+=error;
    }
    if(!valid||totalSteps>rowCount-1)continue;
    for(let startIndex=0;startIndex<=rowCount-1-totalSteps;startIndex++){
      ...
      if(!best||score<best.score)best={origin,pitch,steps:totalSteps,residual:normalizedResidual,score};
    }
  }
  if(!best)return null;
  const rows=Array.from({length:rowCount},(_,index)=>best!.origin+best!.pitch*index);
...
export function rowWindows(rows:readonly number[],height:number):Array<{top:number;bottom:number}>{
  if(!Number.isFinite(height)||height<=0||rows.length===0)return[];
  ...
  if(rows.length>1&&(!Number.isFinite(pitch)||pitch<=0))return[];
