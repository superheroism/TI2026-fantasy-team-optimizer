export interface OtsuWhitenessResult {
  readonly rgba: Uint8ClampedArray;
  readonly threshold: number;
  readonly contrastLow: number;
  readonly contrastHigh: number;
}

function histogram(values: Uint8Array): Uint32Array {
  const out = new Uint32Array(256);
  for (const value of values) out[value] = (out[value] ?? 0) + 1;
  return out;
}

function percentileBin(hist: Uint32Array, target: number): number {
  let cumulative = 0;
...
export function whitenessValues(rgba: Uint8ClampedArray): Uint8Array {
...
export function contrastStretch(values: Uint8Array, lowQuantile = 0.02, highQuantile = 0.98): { values: Uint8Array; low: number; high: number } {
  if (!values.length) return { values: new Uint8Array(), low: 0, high: 255 };
  ...
  for (let i = 0; i < values.length; i++) stretched[i] = Math.max(0, Math.min(255, Math.round(((values[i]! - low) * 255) / span)));
 ...
 */
export function otsuWhitenessRgba(rgba: Uint8ClampedArray): OtsuWhitenessResult {
