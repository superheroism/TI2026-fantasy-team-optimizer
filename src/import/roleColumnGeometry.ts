...
export interface RoleColumnConsensus {
  centers:Record<OcrRole,number>;
  observed:Partial<Record<OcrRole,RoleGeometryCandidate>>;
  inferred:OcrRole[];
  score:number;
}

const strength=(candidate:RoleGeometryCandidate):number=>Math.max(0,Math.min(1,candidate.confidence/100))*Math.max(0,Math.min(1,candidate.similarity));
 ...
 */
export function selectRoleColumnConsensus(
  candidates:readonly RoleGeometryCandidate[],
  width:number,
  height:number,
):RoleColumnConsensus|null{
  ...
  const byRole:Record<OcrRole,RoleGeometryCandidate[]>={core:[],mid:[],support:[]};
  for(const candidate of candidates){if(strength(candidate)>=.42)byRole[candidate.role].push(candidate);}
  for(const role of ['core','mid','support'] as const){
  ...
  let best:RoleColumnConsensus|null=null;
  const consider=(a:RoleGeometryCandidate,b:RoleGeometryCandidate):void=>{
    if(a.x>=b.x)return;
    const ySpread=Math.abs(a.y-b.y);if(ySpread>yTolerance)return;
    const gap=b.x-a.x;if(gap<minGap)return;
    const centers={core:0,mid:0,support:0} as Record<OcrRole,number>;
    const observed:Partial<Record<OcrRole,RoleGeometryCandidate>>={[a.role]:a,[b.role]:b};
    let inferred:OcrRole[]=[];
    if(a.role==='core'&&b.role==='support'){
      if(gap<width*.35)return;
      centers.core=a.x;centers.support=b.x;centers.mid=(a.x+b.x)/2;inferred=['mid'];
    }else if(a.role==='core'&&b.role==='mid'){
      const support=b.x+gap;if(support>width*1.03)return;
      centers.core=a.x;centers.mid=b.x;centers.support=support;inferred=['support'];
    }else if(a.role==='mid'&&b.role==='support'){
      const core=a.x-gap;if(core<width*-.03)return;
      centers.core=core;centers.mid=a.x;centers.support=b.x;inferred=['core'];
    }else return;
    if(!(centers.core<centers.mid&&centers.mid<centers.support))return;
    const fullSpan=centers.support-centers.core;if(fullSpan<width*.34)return;
    ...
    if(!best||score>best.score)best={centers,observed,inferred,score};
  };
  for(const core of byRole.core)for(const mid of byRole.mid)consider(core,mid);
  for(const core of byRole.core)for(const support of byRole.support)consider(core,support);
  for(const mid of byRole.mid)for(const support of byRole.support)consider(mid,support);
  return best;
}


export interface CardColumnAnchor { x:number; y:number; }
 ...
 */
export function selectBalancedCardColumns(
  anchors:readonly CardColumnAnchor[],
  width:number,
  height:number,
):number[]|null{
  if(!Number.isFinite(width)||!Number.isFinite(height)||width<=0||height<=0||anchors.length<9)return null;
  const yTolerance=Math.max(12,height*.025),ordered=[...anchors].filter(anchor=>Number.isFinite(anchor.x)&&Number.isFinite(anchor.y)).sort((a,b)=>a.y-b.y),rows:CardColumnAnchor[][]=[];
  for(const anchor of ordered){
    const row=rows.at(-1),rowY=row?.length?median(row.map(item=>item.y)):Number.NaN;
    if(!row||Math.abs(anchor.y-rowY)>yTolerance)rows.push([anchor]);else row.push(anchor);
  }

  const rowTriples:number[][]=[];
  for(const row of rows){
    const xs=row.map(item=>item.x).sort((a,b)=>a-b);
    if(xs.length<3||xs.at(-1)!-xs[0]!<width*.34)continue;
    let best:{centers:number[];cost:number}|null=null;
    for(let first=1;first<=xs.length-2;first++)for(let second=first+1;second<=xs.length-1;second++){
      ...
      if(leftGap<width*.12||rightGap<width*.12||span<width*.34)continue;
      const balance=Math.min(leftGap,rightGap)/Math.max(leftGap,rightGap);if(balance<.48)continue;
      ...
      if(!best||cost<best.cost)best={centers,cost};
    }
    if(best)rowTriples.push(best.centers);
  }
  if(rowTriples.length<3)return null;

  const supportTolerance=width*.075;
  let seed:number[]|null=null,bestSupport:readonly number[][]=[];
  for(const candidate of rowTriples){
    ...
    if(support.length>bestSupport.length){seed=candidate;bestSupport=support;}
  }
  if(!seed||bestSupport.length<3)return null;
  const centers=[0,1,2].map(index=>median(bestSupport.map(row=>row[index]!)));
  const leftGap=centers[1]!-centers[0]!,rightGap=centers[2]!-centers[1]!,span=centers[2]!-centers[0]!;
  if(span<width*.35||Math.min(leftGap,rightGap)/Math.max(leftGap,rightGap)<.55)return null;
  return centers;
}
