...
function bestTierLine(ls:readonly Line[]):{match:{value:QualityTier;score:number};line?:Line}{let best:{match:{value:QualityTier;score:number};line?:Line}={match:{value:1,score:.2}};for(const line of ls){const match=matchTierText(line.text),hasTier=line.words.some(w=>ocrSimilarity(w.text,'TIER')>=.62),bonus=line.words.some(w=>/^\+?(10|30|60|100|150)%$/.test(w.text.replace(/[“”'`]/g,''))),score=Math.min(.99,match.score+(hasTier ? .08 : 0)+(bonus ? .08 : 0));if(score>best.match.score)best={match:{value:match.value,score},line};}return best;}
...
export async function refineUncertainScreenshotFields(file:File,data:DataBundle,raw:RawScreenshotImport,metrics:LocalScreenshotOcrMetrics):Promise<ScreenshotRefinementResult>{
  const started=performance.now(),src=await image(file),w=await worker(),layout=BOARD_LAYOUTS[raw.layoutId];let retries=0,emblemRetries=0,teamRetries=0,footerRetries=0;
  ...
  for(const role of ROLES){for(let i=0;i<layout.roles[role].length;i++){
    ...
    const tier=bestTierLine(ls),qc=combined(tier.match.score,tier.line?.words??words);if(tier.match.score>=.72&&qc>confidenceFor(raw,qp)){raw.banners[role].emblems[i]!.qualityTier=tier.match.value;setConfidence(raw,qp,qc);d.normalizedTier=tier.match.value;d.tierMatchScore=tier.match.score;}
  ...
  if(raw.operationIds.some(x=>x===null)||raw.tokensRemaining===undefined){const last=rows.at(-1)??metrics.extractionHeight*.58,footerTop=Math.min(metrics.extractionHeight*.82,last+pitch*.72),footerRect=extractionToSource({left:metrics.extractionWidth*.22,top:footerTop,width:metrics.extractionWidth*.72,height:metrics.extractionHeight-footerTop},metrics),rec=await w.recognize(canvas(src,footerRect),{tessedit_pageseg_mode:'6'},{tsv:true}),words=parse(rec.data.tsv),all=orderedText(words);retries++;footerRetries++;
    ...
    const groups=cluster3(words);if(groups){const cardTexts=groups.map(g=>orderedText(g)),resolved:[string|null,string|null,string|null]=[null,null,null];for(let i=0;i<3;i++){const match=matchActionText(cardTexts[i]??'');if(match&&match.score>=.58){resolved[i]=match.id;raw.operationIds[i]=match.id;setConfidence(raw,`operationIds.${i}`,match.score);}}metrics.diagnostic.actionEvidence={resolved,reason:resolved.every(Boolean)?'resolved-by-footer-clustering':'footer-clustering-partial',cardTexts};}else metrics.diagnostic.actionEvidence={resolved:[null,null,null],reason:'footer-action-clustering-underdetermined',cardTexts:[all]};
  }
