...
function traitMatch(s:string){let best={value:TRAITS[0]!,score:-1};for(const v of TRAITS){const score=sim(s,v);if(score>best.score)best={value:v,score};}return best;}
function tierMatch(s:string):{value:QualityTier;score:number}{const bonuses=[...s.matchAll(/([+-]?)(10|30|60|100|150)%/g)].map(m=>Number(m[2])),bonus=bonuses.find(v=>[10,30,60,100,150].includes(v)),byBonus:Record<number,QualityTier>={10:1,30:2,60:3,100:4,150:5};if(bonus&&byBonus[bonus])return{value:byBonus[bonus]!,score:.99};const m=s.toUpperCase().match(/\b(?:III|IV|II|V|I)\b/g)??[],v=m.find(x=>x!=='I')??m[0],map:Record<string,QualityTier>={I:1,II:2,III:3,IV:4,V:5};return v&&map[v]?{value:map[v]!,score:.82}:{value:1,score:.2};}
...
function actionMatch(s:string){return ACTION_CATALOG.map(a=>({id:a.id,score:sim(s,a.label)})).sort((a,b)=>b.score-a.score)[0];}
...
export async function parseScreenshotLocally(file:File,data:DataBundle):Promise<LocalScreenshotOcrOutput>{
  ...
  for(const role of ROLES){const b=bs[role],rs=rows[role],first=rs[0]??ex.height*.2,tw=within(ex.words,b.left,b.left+(b.right-b.left)*.58,0,Math.max(first-5,ex.height*.24)),tm=teamMatch(tw,role,data);teamEvidence[role]={rawText:text(tw),normalizedTeam:tm.team,matchScore:tm.score};fc.push({path:`banners.${role}.selectedTeam`,confidence:tm.score});if(tm.score<.9)warnings.push(`${role} team OCR should be reviewed.`);const emblems=layout.roles[role].map((slot,i)=>{const rw=rowWindow(rs,i,ex.height);synthesizedRows ||= rw.synthesized;const roi={left:b.left+(b.right-b.left)*.42,top:rw.top,width:b.right-(b.left+(b.right-b.left)*.42),height:rw.bottom-rw.top},ww=within(ex.words,roi.left,roi.left+roi.width,roi.top,roi.top+roi.height),s=text(ww),sm=statMatch(s,LEGAL_STAT_POOLS[slot.color]),tr=traitMatch(s),qt=tierMatch(s),sc=conf(sm.score,ww),tc=conf(tr.score,ww),finalConfidence=Math.min(sc,qt.score,tc);fc.push({path:`banners.${role}.emblems.${i}.stat`,confidence:sc},{path:`banners.${role}.emblems.${i}.qualityTier`,confidence:qt.score},{path:`banners.${role}.emblems.${i}.trait`,confidence:tc});if(finalConfidence<.9)warnings.push(`${role} emblem ${i+1} OCR should be reviewed.`);emblemDiagnostics.push({role,rowIndex:i,roi,synthesizedRow:rw.synthesized,words:ww.map(wordDiagnostic),inferredColor:slot.color,rawText:s,normalizedStat:sm.value,statMatchScore:sm.score,rawTierText:s,normalizedTier:qt.value,tierMatchScore:qt.score,rawTraitText:s,normalizedTrait:tr.value,traitMatchScore:tr.score,finalConfidence,reviewRequired:finalConfidence<REVIEW_THRESHOLD});return{position:slot.index,color:slot.color,stat:sm.value,qualityTier:qt.value,trait:tr.value};});banners[role]={selectedTeam:tm.team,emblems};}

  const actionRows=pooled.length?pooled:ROLES.flatMap(r=>rows[r]),actions=await parseActions(worker,img,ex,crop,actionRows,fc,warnings),tokens=tokenCount(ex.words),result:RawScreenshotImport={layoutId,banners,operationIds:actions.ops,fieldConfidence:fc,warnings};if(tokens){result.tokensRemaining=tokens.value;fc.push({path:'tokensRemaining',confidence:tokens.confidence});}
