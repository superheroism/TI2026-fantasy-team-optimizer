...
const ALL_STATS: readonly StatName[] = [...new Set(Object.values(LEGAL_STAT_POOLS).flat())];
...
export interface EmblemOcrDiagnostic {
  ...
  statRunnerUpScore:number;
  statMatchMargin:number;
  rawTierText:string;
...
export interface LocalScreenshotOcrMetrics {
  ...
  ocrExecution:OcrExecutionBudget;
}
...
function canvas(i:HTMLImageElement,max:number,c?:Rect):HTMLCanvasElement{const s=c??{left:0,top:0,width:i.naturalWidth,height:i.naturalHeight},o=document.createElement('canvas');if(validateOcrRect(s,i.naturalWidth,i.naturalHeight)){o.width=0;o.height=0;return o;}const k=Math.min(1,max/Math.max(s.width,s.height));o.width=Math.max(1,Math.round(s.width*k));o.height=Math.max(1,Math.round(s.height*k));const x=o.getContext('2d');if(!x)throw new Error('Canvas image processing is unavailable.');x.drawImage(i,s.left,s.top,s.width,s.height,0,0,o.width,o.height);return o;}
...
async function run(w:Worker,c:HTMLCanvasElement,budget:OcrExecutionBudget,stage:string,crop?:Rect):Promise<Pass>{const t=performance.now(),r=await recognizeWithBudget(w,c,budget,{stage,psm:3,...(crop?{crop}:{}),canvasWidth:c.width,canvasHeight:c.height},{},{tsv:true},resetWorker);return{words:parse(r.data.tsv),elapsedMs:performance.now()-t,width:c.width,height:c.height};}
async function runPsm(w:Worker,c:HTMLCanvasElement,budget:OcrExecutionBudget,stage:string,psm:6|11,crop?:Rect):Promise<Pass>{const t=performance.now();await w.setParameters({tessedit_pageseg_mode:String(psm)});try{const r=await recognizeWithBudget(w,c,budget,{stage,psm,...(crop?{crop}:{}),canvasWidth:c.width,canvasHeight:c.height},{},{tsv:true},resetWorker);return{words:parse(r.data.tsv),elapsedMs:performance.now()-t,width:c.width,height:c.height};}finally{if(!budget.exhausted)await w.setParameters({tessedit_pageseg_mode:'3'});}}
...
interface CenterResult { centers:Record<Role,number>; method:'role-labels'|'card-anchor-clustering'|'fallback'; roleCandidates:RoleCandidateDiagnostic[]; selectedRoleCandidates:RoleCandidateDiagnostic[]; cardAnchors:CardAnchorDiagnostic[]; }
function centerResult(ws:readonly Word[],width:number,height:number):CenterResult{const roleCandidates:RoleCandidateDiagnostic[]=[];for(const r of ROLES){const a=ws.map(w=>({w,similarity:sim(w.text,r.toUpperCase())})).filter(x=>x.similarity>=.72).sort((x,y)=>y.w.confidence-x.w.confidence);for(const x of a)roleCandidates.push({...wordDiagnostic(x.w),role:r,similarity:x.similarity});}const anchors=ws.map(w=>({w,similarity:cardAnchorScore(w)})).filter(x=>x.similarity>=.72).map(x=>({...wordDiagnostic(x.w),similarity:x.similarity}));const consensus=selectRoleColumnConsensus(roleCandidates,width,height);if(consensus){const selectedRoleCandidates=Object.values(consensus.observed).filter(Boolean) as RoleCandidateDiagnostic[];return{centers:consensus.centers,method:'role-labels',roleCandidates,selectedRoleCandidates,cardAnchors:anchors};}const balanced=selectBalancedCardColumns(anchors,width,height),clustered=balanced??cluster3(anchors.map(x=>x.x),width);if(clustered)return{centers:{core:clustered[0]!,mid:clustered[1]!,support:clustered[2]!},method:'card-anchor-clustering',roleCandidates,selectedRoleCandidates:[],cardAnchors:anchors};return{centers:{core:width/6,mid:width/2,support:width*5/6},method:'fallback',roleCandidates,selectedRoleCandidates:[],cardAnchors:anchors};}
function cropBox(p:Pass,sw:number,sh:number){const g=centerResult(p.words,p.width,p.height),c=g.centers,d=Math.min(c.mid-c.core,c.support-c.mid),l=Math.max(0,c.core-d*.62),r=Math.min(p.width,c.support+d*.62),card=p.words.filter(cardAnchor),roleTop=g.selectedRoleCandidates.length?Math.min(...g.selectedRoleCandidates.map(w=>w.y)):undefined,cardTop=card.length?Math.min(...card.map(w=>w.top)):undefined,topEvidence=g.method==='fallback'?undefined:(roleTop??cardTop),t=topEvidence===undefined?0:Math.max(0,topEvidence-Math.max(20,p.height*.05)),sx=sw/p.width,sy=sh/p.height,localization={left:l,top:t,width:r-l,height:p.height-t},source={left:Math.floor(l*sx),top:Math.floor(t*sy),width:Math.ceil((r-l)*sx),height:Math.ceil(sh-t*sy)};return{source,localization,geometry:g};}
...
type RowStream='tier'|'semantic'|'stat'|'trait'|'card';
interface RowResolution { layoutId:BoardLayoutId; rows:number[]; synthesized:boolean; source:RowStream; confidence:number; tierRows:Record<Role,number[]>; }
...
function sparseExpandedRows(rows:readonly number[]):boolean{if(rows.length<3)return false;const sorted=[...rows].sort((a,b)=>a-b),diffs=sorted.slice(1).map((value,index)=>value-sorted[index]!).filter(value=>value>0);if(diffs.length<2)return false;const base=Math.min(...diffs);if(!Number.isFinite(base)||base<=0)return false;let steps=0;for(const diff of diffs){const n=Math.max(1,Math.round(diff/base));if(Math.abs(diff/base-n)>.22)return false;steps+=n;}return steps>=4;}
function resolveRows(ws:readonly Word[],bs:Record<Role,Band>,height:number):RowResolution{const perRole=Object.fromEntries(ROLES.map(role=>[role,rowStreams(ws,bs[role],height)])) as Record<Role,Record<RowStream,number[]>>,streams:RowStream[]=['tier','semantic','stat','trait','card'],global=Object.fromEntries(streams.map(stream=>[stream,clusteredYs(ROLES.flatMap(role=>perRole[role][stream]),Math.max(10,Math.min(22,height*.018)))])) as Record<RowStream,number[]>;const evidence=streams.map(stream=>({stream,evidence:classifyLayoutEvidence(Object.fromEntries(ROLES.map(role=>[role,perRole[role][stream]])) as Record<Role,number[]>,global[stream])})),tierEvidence=evidence.find(row=>row.stream==='tier')!.evidence,expanded=evidence.filter(row=>row.evidence.kind==='expanded_5').sort((a,b)=>b.evidence.confidence-a.evidence.confidence),legacy=evidence.filter(row=>row.evidence.kind==='legacy_3').sort((a,b)=>b.evidence.confidence-a.evidence.confidence),sparseExpanded=sparseExpandedRows(global.tier);let layoutId:BoardLayoutId;if(tierEvidence.kind==='expanded_5'||sparseExpanded)layoutId='expanded_5';else if(tierEvidence.kind==='legacy_3'&&tierEvidence.confidence>=.82)layoutId='legacy_3';else if(expanded.length)layoutId='expanded_5';else if(legacy.length>=2||(legacy.length===1&&legacy[0]!.evidence.confidence>=.9))layoutId='legacy_3';else throw new Error('Screenshot layout is unresolved; review is required.');const rowCount=layoutId==='expanded_5'?5:3,fits=streams.map(stream=>({stream,fit:fitRowLattice(global[stream],rowCount,height)})).filter((row):row is {stream:RowStream;fit:NonNullable<ReturnType<typeof fitRowLattice>>}=>row.fit!==null).sort((a,b)=>b.fit.matchedRows-a.fit.matchedRows||b.fit.confidence-a.fit.confidence);if(!fits.length)throw new Error('Screenshot row geometry is unresolved; review is required.');const broad=fits[0]!,tierAnchored=fitTierAnchoredLattice(global.tier,rowCount,height,broad.fit),best=tierAnchored?{stream:'tier' as RowStream,fit:tierAnchored}:broad;return{layoutId,rows:best.fit.rows,synthesized:best.fit.synthesized,source:best.stream,confidence:best.fit.confidence,tierRows:Object.fromEntries(ROLES.map(role=>[role,perRole[role].tier])) as Record<Role,number[]>};}
const within=(ws:readonly Word[],l:number,r:number,t:number,b:number)=>ws.filter(w=>cx(w)>=l&&cx(w)<r&&cy(w)>=t&&cy(w)<b);
function statMatch(s:string,legal:readonly StatName[]){return matchStatTextWithMargin(s,legal);}
...
function tierMatchWords(ws:readonly Word[]):{value:QualityTier;score:number}{let best:{value:QualityTier;score:number}={value:1,score:.2};for(const line of groups(ws).values()){const match=tierMatch(text(line));if(match.score>best.score)best=match;}return best;}
...
function rowWindow(rows:number[],i:number,height:number){const window=rowWindows(rows,height)[i];return window?{top:window.top,bottom:window.bottom,synthesized:false}:{top:0,bottom:0,synthesized:true};}
...
export async function parseScreenshotLocally(file:File,data:DataBundle):Promise<LocalScreenshotOcrOutput>{
  ...
  if(nativePixels<=DIRECT_NATIVE_MAX_PIXELS){const native=canvas(img,Number.POSITIVE_INFINITY);local=await run(worker,native,budget,'localization',{left:0,top:0,width:img.naturalWidth,height:img.naturalHeight});const box=cropBox(local,img.naturalWidth,img.naturalHeight);crop=box.source;localizationCrop=box.localization;localGeometry=box.geometry;ex=cropRecognizedPass(local,crop);}
  else{const lc=canvas(img,LOCALIZE_MAX);local=await run(worker,lc,budget,'localization',{left:0,top:0,width:img.naturalWidth,height:img.naturalHeight});const box=cropBox(local,img.naturalWidth,img.naturalHeight);crop=box.source;localizationCrop=box.localization;localGeometry=box.geometry;const ec=canvas(img,EXTRACT_MAX,crop);ex=await run(worker,ec,budget,'extraction',crop);}

  let extractionGeometry=centerResult(ex.words,ex.width,ex.height),bs=bandsFromCenters(extractionGeometry.centers,ex.width),resolvedRows:RowResolution;
  try{resolvedRows=resolveRows(ex.words,bs,ex.height);}catch(initialError){if(budget.exhausted)throw initialError;const recoveryCanvas=canvas(img,EXTRACT_MAX,crop),recovery=await runPsm(worker,recoveryCanvas,budget,'layout-recovery',6,crop),recoveryGeometry=centerResult(recovery.words,recovery.width,recovery.height),recoveryBands=bandsFromCenters(recoveryGeometry.centers,recovery.width);try{const recoveredRows=resolveRows(recovery.words,recoveryBands,recovery.height);ex={...recovery,elapsedMs:ex.elapsedMs+recovery.elapsedMs};extractionGeometry=recoveryGeometry;bs=recoveryBands;resolvedRows=recoveredRows;}catch{throw initialError;}}
  const detected=resolvedRows.tierRows,pooledResult={rows:resolvedRows.rows,synthesized:resolvedRows.synthesized},pooled=pooledResult.rows,layoutId=resolvedRows.layoutId,layout=BOARD_LAYOUTS[layoutId],rows=Object.fromEntries(ROLES.map(r=>[r,[...pooled]])) as Record<Role,number[]>;
