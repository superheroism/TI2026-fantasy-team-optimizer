...
function rolePrefixSampleFrontier(role, banner, data, iterations) {
    let cache = sampleFrontierCache.get(data);
    if (!cache) {
        cache = new Map();
        sampleFrontierCache.set(data, cache);
    }
    const key = JSON.stringify({ role, b: banner, n: iterations });
    const prior = cache.get(key);
    if (prior)
        return prior;
    const board = { core: banner, mid: banner, support: banner };
    const ranked = rankTeamsForRole(role, board, data, iterations), out = [];
    for (const prefix of data.titles.prefixes) {
        let best, bestAdjusted = -Infinity, bestPct = 0;
        for (const row of ranked) {
            const pct = titlePrefixBoostPct(data.titles, role, row.team, prefix.id), adjusted = row.expected * (1 + pct / 100);
...
export function evaluateBoardTargetProbabilityFast(board, data, targetScore, iterations = data.simulation.optimizerIterations) {
    ...
    let bestPrefix, bestExpected = -Infinity;
    for (const prefix of data.titles.prefixes) {
        let total = 0, complete = true;
        for (const role of ROLES) {
            const e = frontiers[role].find(x => x.prefixId === prefix.id);
            if (!e) {
                complete = false;
                break;
            }
            total += e.adjustedExpected;
        }
        if (complete && total > bestExpected) {
            bestExpected = total;
    ...
    if (!bestPrefix)
        return 0;
    const entries = ROLES.map(role => frontiers[role].find(x => x.prefixId === bestPrefix)).filter((x) => Boolean(x));
    if (entries.length !== 3)
        return 0;
    let hits = 0;
    for (let i = 0; i < iterations; i++) {
        ...
        for (const e of entries)
            total += (e.row.samples[i] ?? e.row.expected) * (1 + e.boostPct / 100);
