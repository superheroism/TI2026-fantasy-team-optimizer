...
function cluster(values, tol) { const s = [...values].sort((a, b) => a - b), out = []; for (const v of s) {
    if (!out.length || v - out.at(-1) > tol)
        out.push(v);
    else
        out[out.length - 1] = (out.at(-1) + v) / 2;
} return out; }
...
export async function refineUncertainEmblemStats(file, data, raw) {
    ...
    for (const role of ROLES)
        for (let i = 0; i < layout.roles[role].length; i++) {
            const path = `banners.${role}.emblems.${i}.stat`;
            if (confidenceFor(raw, path) < .9)
                targets.push({ role, index: i, color: layout.roles[role][i].color, path });
        }
    if (!targets.length)
        return raw;
    ...
    for (const t of targets) {
        ...
        if (match.score >= .58 && confidence > confidenceFor(raw, t.path)) {
            raw.banners[t.role].emblems[t.index].stat = match.value;
            setConfidence(raw, t.path, confidence);
        }
    }
    return raw;
}
