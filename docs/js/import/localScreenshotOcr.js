...
function globalRows(ws, bs, height) { const per = ROLES.map(r => tiers(ws, bs[r], height)), all = clusteredYs(per.flat(), Math.max(10, Math.min(22, height * .018))); if (all.length >= 5)
    return { rows: all.slice(0, 5), synthesized: false }; if (all.length === 4) {
    ...
    for (let i = 0; i < diffs.length; i++)
        if (diffs[i] > pitch * 1.55)
            gap = i;
    if (gap >= 0)
        return { rows: [...all.slice(0, gap + 1), (all[gap] + all[gap + 1]) / 2, ...all.slice(gap + 1)].slice(0, 5), synthesized: true };
    return { rows: [...all, all.at(-1) + pitch], synthesized: true };
} return { rows: all, synthesized: false }; }
