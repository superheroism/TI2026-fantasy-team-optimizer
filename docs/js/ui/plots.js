...
export function renderComparisonTabs(role, onChange) {
    ...
    document.querySelectorAll('[data-compare]').forEach(button => button.addEventListener('click', () => onChange(button.dataset.compare)));
}
export function renderTeamComparison(role, board, data) {
...
export function clearHistogram(message) {
    const canvas = $('#hist');
...
export function drawHistogram(samples, target, expected, median, p10, p90) {
    const canvas = $('#hist');
    const ctx = canvas.getContext('2d');
    const rect = canvas.getBoundingClientRect();
    const dpr = devicePixelRatio || 1;
    ...
    const xPos = (value) => left + (value - min) / (max - min) * plotWidth;
    ctx.strokeStyle = cssVar('--chart-grid', '#33283f');
    ctx.lineWidth = 1;
    ctx.beginPath();
    ctx.moveTo(left, height - bottom + .5);
    ctx.lineTo(width - right, height - bottom + .5);
    ctx.stroke();
    ...
    for (let tick = first; tick <= max; tick += step) {
        ...
        ctx.stroke();
        ctx.fillText(fmt(tick), x, height - 12);
    }
    const marker = (value, color, label, dash = [], labelY = top + 10) => {
        ...
        ctx.textAlign = x > width - 100 ? 'right' : 'left';
        ctx.fillText(label, x + (x > width - 100 ? -5 : 5), labelY);
        ctx.restore();
    };
