...
export function groundTruthCanonical(gt){
  ...
  for(const role of ROLES){
    const label=roleLabel(role),team=gt.selectedTeams?.[label]??gt.selectedTeams?.[role]??null;
    out.banners[role]={selectedTeam:typeof team==='object'?team.expected:team,emblems:(gt.banners?.[label]??gt.banners?.[role]??[]).map((e,i)=>({position:i+1,stat:e.stat,qualityTier:e.tier,trait:e.trait}))};
  }
  return out;
}
...
export function flattenCanonical(value){
  ...
  fields.set('layoutId',value?.layoutId);
  for(const role of ROLES){
  ...
  fields.set('tokensRemaining',value?.tokensRemaining);
  return fields;
}
...
export function compareStages({groundTruth,raw,validated,applied,rendered,sentinel}){
  const expected=groundTruthCanonical(groundTruth);
  const maps={expected:flattenCanonical(expected),raw:flattenCanonical(raw),validated:flattenCanonical(canonicalFromValidated(validated)),applied:flattenCanonical(canonicalFromApplied(applied)),rendered:flattenCanonical(rendered),sentinel:flattenCanonical(sentinel)};
  const conf=confidenceByPath(raw),mismatches=[];
  let falseHighConfidenceErrors=0,rawCorrectFinalWrong=0;
  for(const [fieldPath,expectedValue] of maps.expected){
    const teamSpec=fieldPath.endsWith('.selectedTeam')?groundTruth.selectedTeams?.[roleLabel(fieldPath.split('.')[1])]:undefined;
    const unmappable=teamSpec&&typeof teamSpec==='object'&&teamSpec.expectedReview===true;
    const values={expected:expectedValue,raw:maps.raw.get(fieldPath),validated:maps.validated.get(fieldPath),applied:maps.applied.get(fieldPath),rendered:maps.rendered.get(fieldPath),sentinel:maps.sentinel.get(fieldPath)};
    const firstDivergence=classifyField({...values,unmappable});
    if(firstDivergence){
      const rawConfidence=conf.get(fieldPath);
      mismatches.push({path:fieldPath,...values,rawConfidence,firstDivergence});
      if(firstDivergence===DIVERGENCE.RAW_OCR_ERROR&&typeof rawConfidence==='number'&&rawConfidence>=0.9) falseHighConfidenceErrors++;
    }
...
export async function withWatchdog(factory,timeoutMs=30000,label='operation'){
  let timer;
  try{return await Promise.race([factory(),new Promise((_,reject)=>{timer=setTimeout(()=>{const e=new Error(`${label} timed out after ${timeoutMs} ms`);e.code='E_WATCHDOG';reject(e);},timeoutMs);})]);}
...
function webpDimensions(buf){
  ...
  if(kind==='VP8L'){const b1=buf[21],b2=buf[22],b3=buf[23],b4=buf[24];return {width:1+b1+((b2&0x3f)<<8),height:1+(b2>>6)+(b3<<2)+((b4&0x0f)<<10)};}
  if(kind==='VP8 '){const marker=buf.indexOf(Buffer.from([0x9d,0x01,0x2a]),20);if(marker>=0&&marker+7<buf.length)return {width:buf.readUInt16LE(marker+3)&0x3fff,height:buf.readUInt16LE(marker+5)&0x3fff};}
  return null;
}
...
export function summarizeSafety(localOcrMetrics){
  const text=JSON.stringify(localOcrMetrics??{}).toLowerCase();
  const num=(...names)=>{for(const n of names){const v=localOcrMetrics?.[n];if(Number.isFinite(v))return v;}return 0;};
  return {timeouts:num('timeoutCount','ocrTimeoutCount'),invalidGeometry:num('invalidRoiCount','invalidGeometryCount'),diagnosticMentionsTimeout:text.includes('timeout')};
}
