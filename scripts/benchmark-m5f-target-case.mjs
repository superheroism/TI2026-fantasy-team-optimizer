...
if (!caseId || !Number.isInteger(horizon) || horizon < 1 || !CONTINUATION_FIDELITY_PRESETS[fidelityId] || (wideningId !== 'none' && !ACTION_WIDENING_PRESETS[wideningId])) {
  throw new Error('Usage: benchmark-m5f-target-case.mjs <case-id> <horizon> <current|aggressive> <none|wide>');
}
if (!['current', 'aggressive'].includes(fidelityId) || !['none', 'wide'].includes(wideningId)) {
  throw new Error('M5F only permits current/aggressive fidelity and none/wide widening.');
}
...
const memorySnapshot = () => {
  ...
  return { heapUsed: u.heapUsed, heapTotal: u.heapTotal, rss: u.rss, external: u.external, arrayBuffers: u.arrayBuffers };
};
...
function run() {
  ...
  const state = {
    ...
    tokensRemaining: 10,
    menu: MENU_IDS.map(action),
    menuRerollAvailable: true,
    username: 'M5F target 55k',
  ...
  const stop = ranking.find((row) => row.key === 'stop');
  const menuReroll = ranking.find((row) => row.key === 'menu_reroll');
  const output = {
    id: caseId,
    horizon,
    fidelityId,
    ...
    stopValue: stop?.utility ?? null,
    menuRerollValue: menuReroll?.utility ?? null,
    ranking,
    memory: {
      start: startMemory,
      end: endMemory,
      ...
      maxRssStartKb,
      maxRssEndKb: process.resourceUsage().maxRSS,
    },
