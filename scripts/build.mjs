...
if(!Array.isArray(model.levels)||model.levels.length!==104)throw new Error('Statistical model percentile ladder is incomplete.');
for(const role of ['Core','Mid','Support']){
  const r=model.roles?.[role], c=model.gcorr?.[role];
  if(!r?.teams?.length||!r?.stats?.length||!r?.cells)throw new Error(`Statistical model ${role} data is incomplete.`);
  if(!c?.stats?.length||!Array.isArray(c.m)||c.m.length!==c.stats.length)throw new Error(`Statistical model ${role} correlation matrix is incomplete.`);
...
if(!Array.isArray(titles.prefixes)||titles.prefixes.length!==8)throw new Error('Title model must contain the eight title prefixes.');
if(!Array.isArray(titles.suffixes)||!titles.suffixes.some(s=>s.id===titles.fixedSuffixId))throw new Error('Title model fixed suffix is invalid.');
for(const role of ['core','mid','support']){
  const byTeam=titles.prefixBoostPctByRoleTeam?.[role];
  if(!byTeam||Object.keys(byTeam).length!==16)throw new Error(`Title model ${role} table must contain 16 teams.`);
...
console.log('Built static site in docs/ with committed statistical and title model snapshots.');
